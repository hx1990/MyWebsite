...
const multerObj=multer({dest:'./libs/pic'})
    ...
    switch(req.query.act){    //根据请求传的act不同做不同的操作
       ...
       db.query(`SELECT * FROM banners WHERE ID=${req.query.id}`,(err,data)=>{
           ...
           if(err){
               ...
               db.query(`SELECT * FROM banners`,(err,banners)=>{//获取所有数据
                  if(err){                                                      
                      console.error(err)
                      res.status(500).send('datadase err').end()
                  }else{
                      ...
                      res.render('admin/banners.ejs',{banners1,mod_data:data[0]})
                  } 
               })
           }
       })
       ...
       db.query(`SELECT * FROM banners WHERE ID=${req.query.id}`,(err,data)=>{
           if(err){
               ...
               res.status(404).send('no this banners opration').end()
           }else{
               fs.unlink('libs/pic/'+path.parse(data[0].src).base,(err)=>{
                    if(err){
                        console.error(err)
       ...
       db.query(`SELECT * FROM banners`,(err,banners)=>{
           if(err){
               ...
               res.render('admin/banners.ejs',{banners1})
           }
       })
    }
})
  ...
  if(req.body.mod_id){//修改数据
    if(newFileName){  //上传文件
     fs.rename(oldpath,newpath,(err)=>{   //修改上传的文件名后缀
        if(err){
            ...
            db.query(`SELECT * FROM banners WHERE ID=${req.body.mod_id}`,(err,data)=>{
                if(err){
                    console.error(err)
                    res.status(500).send('database err').end()
                }else if(data.length==0){
                    ...
                    fs.unlink('libs/pic/'+path.parse(data[0].src).base,(err)=>{
                        if(err){
                            ...
                            db.query(`UPDATE banners SET title="${title}", src="./server/libs/pic/${newFileName}" WHERE ID=${req.body.mod_id}`,(err,data)=>{
                                if(err){
                                    console.error(err)
                                    res.status(500).send('database err2').end()
                                }else{
      ...
      db.query(`UPDATE banners SET title='${title}' WHERE ID=${req.body.mod_id}`,(err,data)=>{
          if(err){
              console.error(err)
              res.status(500).send('database err').end()
          }else{
    ...
    fs.rename(oldpath,newpath,(err)=>{   //修改上传的文件名后缀
        if(err){
            ...
            db.query(`INSERT INTO banners (title,src) VALUES("${title}","./server/libs/pic/${newFileName}")`,(err,data)=>{
                if(err){
                    console.error(err)
